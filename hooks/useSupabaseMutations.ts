  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        estimated_receive_date: po.estimatedReceiveDate ?? null,
        total: po.total,
        taxable_discount_fee_freight: po.taxableFeeFreight ?? 0,
    ...
    const poId = data?.id;
    if (!poId) throw new Error('Unable to determine purchase order ID.');
...
export async function updatePurchaseOrderStatus(
  ...
  try {
    const { error } = await supabase
      .from('purchase_orders')
      .update({ status, record_last_updated: new Date().toISOString() })
      .eq('id', id);
