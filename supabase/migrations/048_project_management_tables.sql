...
CREATE TABLE IF NOT EXISTS delegation_settings (
  ...
  assignable_to_roles TEXT[] DEFAULT ARRAY['Admin', 'Manager', 'Staff'],
...
-- Ensure legacy rows have required ownership + scheduling metadata
ALTER TABLE projects
  ADD COLUMN IF NOT EXISTS delegate_id UUID REFERENCES auth.users(id);

UPDATE projects
SET delegate_id = COALESCE(delegate_id, default_assignee_id, owner_id);

UPDATE projects
SET start_date = COALESCE(start_date, CURRENT_DATE);

UPDATE projects
SET target_end_date = COALESCE(target_end_date, start_date + INTERVAL '14 days');

-- Fallback owner assignment (use default assignee, otherwise first user)
...
UPDATE projects
SET owner_id = COALESCE(owner_id, default_assignee_id, fallback_owner.user_id)
FROM fallback_owner
WHERE owner_id IS NULL;

ALTER TABLE projects
  ALTER COLUMN start_date SET NOT NULL,
  ALTER COLUMN target_end_date SET NOT NULL;

DO $$
DECLARE missing_owners INTEGER;
BEGIN
  SELECT COUNT(*) INTO missing_owners FROM projects WHERE owner_id IS NULL;
  IF missing_owners = 0 THEN
    ALTER TABLE projects
      ALTER COLUMN owner_id SET NOT NULL;
  ELSE
    RAISE EXCEPTION 'Unable to enforce NOT NULL on projects.owner_id — % row(s) still missing owners.', missing_owners;
  END IF;
END $$;

DO $$
DECLARE missing_delegates INTEGER;
BEGIN
  SELECT COUNT(*) INTO missing_delegates FROM projects WHERE delegate_id IS NULL;
  IF missing_delegates = 0 THEN
    ALTER TABLE projects
      ALTER COLUMN delegate_id SET NOT NULL;
  ELSE
    RAISE EXCEPTION 'Unable to enforce NOT NULL on projects.delegate_id — % row(s) still missing delegates.', missing_delegates;
  END IF;
END $$;
...
BEGIN
  ALTER TABLE projects
    ADD CONSTRAINT projects_dates_chk CHECK (target_end_date >= start_date);
EXCEPTION
...
CREATE TABLE IF NOT EXISTS tickets (
  ...
  -- Assignment
  reporter_id UUID NOT NULL REFERENCES auth.users(id),  -- Who created the ticket
...
UPDATE tickets
SET reporter_id = COALESCE(reporter_id, fallback_reporter.user_id)
FROM fallback_reporter
...
WITH fallback_comment_author AS (
  SELECT id AS user_id
  FROM auth.users
  ORDER BY created_at
  LIMIT 1
...
SET author_id = COALESCE(author_id, fallback_comment_author.user_id)
FROM fallback_comment_author
...
WITH fallback_activity_actor AS (
  SELECT id AS user_id
  FROM auth.users
  ORDER BY created_at
  LIMIT 1
...
SET actor_id = COALESCE(actor_id, fallback_activity_actor.user_id)
FROM fallback_activity_actor
...
VALUES 
  ('question', ARRAY['Admin', 'Manager', 'Staff'], ARRAY['Admin', 'Manager', 'Staff'], ARRAY['Admin', 'Manager', 'Staff'], false, NULL, true),
  ...
  ('bom_revision_approval', ARRAY['Admin', 'Manager'], ARRAY['Admin'], ARRAY['Admin', 'Manager', 'Staff'], true, ARRAY['Manager', 'Admin'], true),
  ('artwork_approval', ARRAY['Admin', 'Manager', 'Staff'], ARRAY['Admin', 'Manager'], ARRAY['Admin', 'Manager'], true, ARRAY['Manager'], true),
  ('po_approval', ARRAY['Admin', 'Manager'], ARRAY['Admin'], ARRAY['Admin', 'Manager', 'Staff'], true, ARRAY['Manager', 'Admin'], true),
  ('requisition_approval', ARRAY['Admin', 'Manager', 'Staff'], ARRAY['Admin', 'Manager'], ARRAY['Manager', 'Admin', 'Staff'], true, ARRAY['Manager', 'Admin'], true),
