...
CREATE INDEX idx_purchase_orders_finale_sync ON purchase_orders(last_finale_sync);
...
CREATE INDEX idx_po_items_inventory_sku ON purchase_order_items(inventory_sku);
CREATE INDEX idx_po_items_line_status ON purchase_order_items(line_status);
...
CREATE INDEX idx_reorder_queue_sku ON reorder_queue(inventory_sku);
CREATE INDEX idx_reorder_queue_vendor ON reorder_queue(vendor_id);
CREATE INDEX idx_reorder_queue_status ON reorder_queue(status);
CREATE INDEX idx_reorder_queue_urgency ON reorder_queue(urgency);
CREATE INDEX idx_reorder_queue_priority ON reorder_queue(priority_score DESC);
CREATE INDEX idx_reorder_queue_identified ON reorder_queue(identified_at DESC);
CREATE INDEX idx_reorder_queue_active ON reorder_queue(status)
...
CREATE INDEX idx_finale_sync_log_entity ON finale_sync_log(entity_type, entity_id);
CREATE INDEX idx_finale_sync_log_started ON finale_sync_log(started_at DESC);
CREATE INDEX idx_finale_sync_log_status ON finale_sync_log(status);
...
-- Uncomment to insert sample PO
-- INSERT INTO purchase_orders (
--   order_id, supplier_name, status, expected_date, total_amount
-- ) VALUES (
--   'PO-20251117-001', 'Sample Vendor', 'draft', CURRENT_DATE + INTERVAL '14 days', 1000.00
-- );
